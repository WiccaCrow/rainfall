...
# 8.2.1. Подготовка и вызов вызов :
...
Достаточно длинная функция main(). И это единственная не библиотечная функция программы.

`... <+394> ... <+401> ` вызов system("/bin/sh"). \
Значит, надо понять, как попасть в это место.
