...
0x08048ec6 <+6>:     sub    $0x20,%esp        # выделяется 20 (hex) = 32 (dec) байт под локальные переменные
...
# в eax помещается значение `esp+10` (без разименования)(https://stackoverflow.com/questions/1658294/whats-the-purpose-of-the-lea-instruction), то есть адрес на буфер в 50(hex)-10(hex)=40(hex) = 64(dec) байта 
...
Последние строки в disassemble main:
...
# 6. Завершение и возврат из функции:
0x080484a5 <+78>:    leave  
0x080484a6 <+79>:    ret    
# End of assembler dump.
```
...
* Заполню оставшиеся 0x080484a4<sub>16</sub>-4<sub>10</sub>=134513828<sub>10</sub>-4<sub>10</sub>=134513824 байт задав ширину поля `%134513824d`.
```sh
...
<details><summary> getfacl level6 </summary>
